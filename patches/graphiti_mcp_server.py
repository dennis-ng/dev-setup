...
mcp = FastMCP(
    ...
    # Disable MCP DNS-rebinding host check. FastMCP auto-enables it (localhost-only
    # allowed_hosts) because it's constructed before settings.host is set to 0.0.0.0,
    # which 421s remote Host headers (e.g. Tailscale-served hostname). Safe here:
    # endpoint is loopback-bound, reached only over the tailnet (WireGuard-authed),
    # and the client is a CLI, not a browser. Explicit setting also skips auto-enable.
